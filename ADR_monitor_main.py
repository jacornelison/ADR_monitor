...
def init_dock_and_plot(area):
    dock_list = []
    plot_list = []
    
    for kidx,k in enumerate(cg.monitor_channels.keys()):
        dock_list.append(Dock(name=k))
        plot_list.append(pg.PlotWidget())
        dock_list[kidx].addWidget(plot_list[kidx])
        area.addDock(dock_list[kidx])
    
    return plot_list

plot_list = init_dock_and_plot(area)
